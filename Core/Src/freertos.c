...
osEventFlagsId_t SystemReadyFlagsHandle;
...
void MX_FREERTOS_Init(void) {
  ...
  (void)osThreadNew(Can_Task_Entry, NULL, &CanTask_attributes);
  ...
  SystemReadyFlagsHandle = osEventFlagsNew(NULL);
...
void Hearbeat_Task(void *argument)
{
  ...
  for(;;)
  {
    ...
    // 诊断：每10秒打印各任务栈的历史�?小剩余量(高水位线)，单位字�?
    // 数�?�会随着各任务跑到过的最坏情�?(OLED刷新/DMP解算/UART突发�?)逐渐变小并趋于稳�?
    hwm_counter++;
    if(hwm_counter >= 10)
    {
        hwm_counter = 0;
